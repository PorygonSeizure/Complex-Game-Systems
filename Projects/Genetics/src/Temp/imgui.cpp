...
!- popups/menus: clarify usage of popups id, how MenuItem/Selectable closing parent popups affects the ID, etc. this is quite fishy needs improvement! (#331, #402)
 ...
 - slider: tint background based on value (e.g. v_min -> v_max, or use 0.f either side of the sign)
...
bool ImGui::BeginChild(const char* strID, const ImVec2& size_arg, bool border, ImGuiWindowFlags extraFlags)
{
	ImGuiWindow* window = GetCurrentWindow();
	...
	if (size.x <= 0.f)
	{
		...
		size.x = ImMax(content_avail.x, 4.f) - fabsf(size.x);	//Arbitrary minimum zero-ish child size of 4.f (0.f causing too much issues)
	}
	if (size.y <= 0.f)
	{
		if (size.y == 0.f)
			flags |= ImGuiWindowFlags_ChildWindowAutoFitY;
		size.y = ImMax(content_avail.y, 4.f) - fabsf(size.y);
	}
	if (border)
...
static ImVec2 FindBestPopupWindowPos(const ImVec2& basePos, const ImVec2& size, ImGuiWindowFlags flags, int* lastDir, const ImRect& rInner)
{
	...
	ImVec2 basePos_clamped = ImClamp(basePos, r_outer.Min, r_outer.Max - size);

	for (int n = (*lastDir != -1) ? -1 : 0; n < 4; n++)   //Right, down, up, left. Favor last used direction.
	{
		const int dir = (n == -1) ? *lastDir : n;
		ImRect rect(dir == 0 ? rInner.Max.x : r_outer.Min.x, dir == 1 ? rInner.Max.y : r_outer.Min.y, dir == 3 ? rInner.Min.x : r_outer.Max.x, dir == 2 ? rInner.Min.y : r_outer.Max.y);
		if (rect.GetWidth() < size.x || rect.GetHeight() < size.y)
			continue;
		*lastDir = dir;
		return ImVec2(dir == 0 ? rInner.Max.x : dir == 3 ? rInner.Min.x - size.x : basePos_clamped.x, dir == 1 ? rInner.Max.y : dir == 2 ? rInner.Min.y - size.y : basePos_clamped.y);
	}
...
bool ImGui::Begin(const char* name, bool* pOpened, ImGuiWindowFlags flags)
{
	return ImGui::Begin(name, pOpened, ImVec2(0.f, 0.f), -1.f, flags);
}

bool ImGui::Begin(const char* name, bool* pOpened, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	ImGuiState& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	...
	if (flags & ImGuiWindowFlags_Popup)
	{
		...
		window_was_active &= (window->PopupID == popupRef.PopupID);
		window_was_active &= (window == popupRef.Window);
	...
	int root_idx, root_non_popupIDX;
	...
	for (root_non_popupIDX = root_idx; root_non_popupIDX > 0; root_non_popupIDX--)
	{
		if (!(g.CurrentWindowStack[root_non_popupIDX]->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_Popup)))
			break;
	}
	window->RootWindow = g.CurrentWindowStack[root_idx];
	window->RootNonPopupWindow = g.CurrentWindowStack[root_non_popupIDX];	//This is merely for displaying the TitleBgActive color.
	...
	if (first_begin_of_the_frame)
	{
		...
		window->SizeContents.x = (window->SizeContentsExplicit.x != 0.f) ? window->SizeContentsExplicit.x : ((window_is_new ? 0.f : window->DC.CursorMaxPos.x - window->Pos.x) + window->Scroll.x);
		window->SizeContents.y = (window->SizeContentsExplicit.y != 0.f) ? window->SizeContentsExplicit.y : ((window_is_new ? 0.f : window->DC.CursorMaxPos.y - window->Pos.y) + window->Scroll.y);
		...
		if ((flags & ImGuiWindowFlags_Tooltip) != 0)
		{
			...
			size_auto_fit = window->SizeContents + window->WindowPadding - ImVec2(0.f, style.ItemSpacing.y);
		}
		else
		{
			...
			size_auto_fit.y = ImMax(size_auto_fit.y - style.ItemSpacing.y, 0.f);
		}
		...
		{
			if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && g.IO.DisplaySize.x > 0.f && g.IO.DisplaySize.y > 0.f) //Ignore zero-sized display explicitly to avoid losing positions if a window manager reports zero-sized window when initializing or minimizing.
			{
		...
		if (window->ScrollTarget.y < FLT_MAX)
		{
			...
			window->Scroll.y = window->ScrollTarget.y - ((1.f - center_ratio) * window->TitleBarHeight()) - (center_ratio * window->SizeFull.y);
		...
		{
			ImU32 resize_col = 0;
			const float resize_corner_size = ImMax(g.FontSize * 1.35f, window_rounding + 1.f + g.FontSize * 0.2f);
			...
			window->ScrollbarX = (window->SizeContents.x > window->Size.x - (window->ScrollbarY ? style.ScrollbarSize : 0.f) - window->WindowPadding.x) && !(flags & ImGuiWindowFlags_NoScrollbar) && (flags & ImGuiWindowFlags_HorizontalScrollbar);
			...
			if (flags & ImGuiWindowFlags_MenuBar)
			{
				...
				window->DrawList->AddRectFilled(menu_bar_rect.GetTL(), menu_bar_rect.GetBR(), window->Color(ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.f, 1 | 2);
			}
			...
			{
				const ImVec2 br = window->Rect().GetBR();
				window->DrawList->PathLineTo(br + ImVec2(-resize_corner_size, 0.f));
				window->DrawList->PathLineTo(br + ImVec2(0.f, -resize_corner_size));
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			ImVec2 text_max = window->Pos + ImVec2(window->Size.x - style.FramePadding.x, style.FramePadding.y * 2 + textSize.y);
			ImVec2 clipMax = ImVec2(window->Pos.x + window->Size.x - (pOpened ? title_bar_rect.GetHeight() - 3 : style.FramePadding.x), text_max.y);	//Match the size of CloseWindowButton()
			bool pad_left = (flags & ImGuiWindowFlags_NoCollapse) == 0;
			bool pad_right = (pOpened != NULL);
			...
			RenderTextClipped(text_min, text_max, name, NULL, &textSize, style.WindowTitleAlign, NULL, &clipMax);
		}
	...
	ImRect clipRect(title_bar_rect.Min.x + 0.5f + window->WindowPadding.x*0.5f, title_bar_rect.Max.y + window->MenuBarHeight() + 0.5f, window->Pos.x + window->Size.x - window->WindowPadding.x*0.5f, window->Pos.y + window->Size.y);
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
	...
	const float border_offset = (window->Flags & ImGuiWindowFlags_ShowBorders) ? 1.f : 0.f;
	ImRect bb = horizontal ? ImRect(window->Pos.x + border_offset, window_rect.Max.y - style.ScrollbarSize, window_rect.Max.x - other_scrollbar_size_w, window_rect.Max.y)
	...
	if (!horizontal)
		bb.Min.y += window->TitleBarHeight() + ((window->Flags & ImGuiWindowFlags_MenuBar) ? window->MenuBarHeight() - border_offset : 0.f);
...
static void PushMultiItemsWidths(int components, float wFull)
{
	...
	const float w_item_one = ImMax(1.f, (float)(int)((wFull - (style.FramePadding.x*2.f + style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
	const float w_item_last = ImMax(1.f, (float)(int)(wFull - (w_item_one + style.FramePadding.x*2.f + style.ItemInnerSpacing.x) * (components - 1)));
...
float ImGui::CalcItemWidth()
{
	...
	if (w < 0.f)
	{
		...
		w = ImMax(1.f, width_to_right_edge + w - g.Style.FramePadding.x * 2.f);
	}
	w = (float)(int)w;
	return w;
}
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
	...
	if (center_y_ratio <= 0.f && window->ScrollTarget.y <= window->WindowPadding.y)	//Minor hack to make "scroll to top" take account of WindowPadding, else it would scroll to (WindowPadding.y - ItemSpacing.y)
...
{
	ImGuiWindow* window = GetCurrentWindow();
	float target_y = window->DC.CursorPosPrevLine.y + (window->DC.PrevLineHeight * center_y_ratio) + (GImGui->Style.ItemSpacing.y * (center_y_ratio - 0.5f) * 2.f);	//Precisely aim above, in the middle or below the last line.
...
void ImGui::TextUnformatted(const char* text, const char* textEnd)
{
	...
	const bool wrap_enabled = wrapPosX >= 0.f;
	if (textEnd - text > 2000 && !wrap_enabled)
	{
		//Long text!
		...
		const ImVec2 text_pos = window->DC.CursorPos + ImVec2(0.f, window->DC.CurrentLineTextBaseOffset);
		const ImRect clipRect = window->ClipRect;
		ImVec2 textSize(0, 0);

		if (text_pos.y <= clipRect.Max.y)
		{
			ImVec2 pos = text_pos;
			...
			if (!g.LogEnabled)
			{
				int lines_skippable = (int)((clipRect.Min.y - text_pos.y) / line_height);
				if (lines_skippable > 0)
				{
					int lines_skipped = 0;
					while (line < textEnd && lines_skipped < lines_skippable)
					{
			...
			if (line < textEnd)
			{
				...
				while (line < textEnd)
				{
					...
					const ImVec2 line_size = CalcTextSize(line, lineEnd, false);
					textSize.x = ImMax(textSize.x, line_size.x);
		...
		ImRect bb(text_pos, text_pos + textSize);
		ItemSize(bb);
		ItemAdd(bb, NULL);
	}
	else
	{
		const float wrapWidth = wrap_enabled ? CalcWrapWidthForPos(window->DC.CursorPos, wrapPosX) : 0.f;
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
	...
	const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + style.FramePadding.x * 2 + (label_size.x > 0.f ? style.ItemInnerSpacing.x : 0.f), style.FramePadding.y * 2) + label_size);
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, NULL))
		return;

	//Render
	const char* value_textBegin = &g.TempBuffer[0];
	const char* value_textEnd = value_textBegin + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
	RenderTextClipped(value_bb.Min, value_bb.Max, value_textBegin, value_textEnd, NULL, ImGuiAlign_VCenter);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.f, label_size.y + style.FramePadding.y * 2.f);
...
bool ImGui::InvisibleButton(const char* strID, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
static bool CloseWindowButton(bool* pOpened)
{
	...
	const float cross_extent = (size * 0.5f * 0.7071f) - 1.f;
	if (hovered)
	{
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	...
	if (border_col.w > 0.f)
	{
		window->DrawList->AddRect(bb.Min, bb.Max, window->Color(border_col), 0.f);
...
bool ImGui::CollapsingHeader(const char* label, const char* strID, bool display_frame, bool default_open)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImRect bb = ImRect(posMin, ImVec2(posMax.x, posMin.y + label_size.y));
	if (display_frame)
	{
	...
	const ImRect text_bb(bb.Min, bb.Min + ImVec2(collapser_width + style.FramePadding.x * 2 * 0 + (label_size.x > 0.f ? label_size.x : 0.f), label_size.y));
	ItemSize(ImVec2(text_bb.GetSize().x, bb.GetSize().y), display_frame ? style.FramePadding.y : 0.f);

	const ImRect interact_bb = display_frame ? bb : ImRect(text_bb.Min, text_bb.Max + ImVec2(style.FramePadding.x * 2, 0.f));	//FIXME
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
	...
	const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(line_height + (label_size.x > 0.f ? (style.FramePadding.x * 2) : 0.f), 0) + label_size);	//Empty text doesn't add padding
	ItemSize(bb);
	if (!ItemAdd(bb, NULL))
		return;
...
void ImGui::PushID(const char* strID_begin, const char* strID_end)
{
	ImGuiWindow* window = GetCurrentWindow();
	window->IDStack.push_back(window->GetID(strID_begin, strID_end));
}
...
ImGuiID ImGui::GetID(const char* strID)
{
	return GImGui->CurrentWindow->GetID(strID);
}

ImGuiID ImGui::GetID(const char* strID_begin, const char* strID_end)
{
	return GImGui->CurrentWindow->GetID(strID_begin, strID_end);
}
...
static void DataTypeApplyOpFromText(const char* buf, const char* initialValueBuf, ImGuiDataType dataType, void* dataPtr, const char* scalarFormat)
{
	...
	if (dataType == ImGuiDataType_Int)
	{
		...
		int* v = (int*)dataPtr;
		int ref_v = *v;
		if (op && sscanf(initialValueBuf, scalarFormat, &ref_v) < 1)
			return;
		...
		{
			if (sscanf(buf, "%f", &op_v) == 1 && op_v != 0.f)
				*v = (int)(ref_v / op_v);
		}	//Divide
		else
		{
			if (sscanf(buf, scalarFormat, &ref_v) == 1)
				*v = ref_v;
		}	//Assign constant
	}
	else if (dataType == ImGuiDataType_Float)
	{
		...
		if (op && sscanf(initialValueBuf, scalarFormat, &ref_v) < 1)
			return;
		float op_v = 0.f;
		if (sscanf(buf, scalarFormat, &op_v) < 1)
			return;

		if (op == '+')
			*v = ref_v + op_v;	//Add (use "+-" to subtract)
		else if (op == '*')
...
float ImGui::RoundScalar(float value, int decimalPrecision)
{
	...
	float min_step = (decimalPrecision >= 0 && decimalPrecision < 10) ? min_steps[decimalPrecision] : powf(10.f, (float)-decimalPrecision);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimalPrecision, ImGuiSliderFlags flags)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	const bool is_non_linear = fabsf(power - 1.f) > 0.0001f;
	const bool is_horizontal = (flags & ImGuiSliderFlags_Vertical) == 0;

	const float grab_padding = 2.f;
	const float slider_sz = is_horizontal ? (frame_bb.GetWidth() - grab_padding * 2.f) : (frame_bb.GetHeight() - grab_padding * 2.f);
	float grab_sz;
	if (decimalPrecision > 0)
		grab_sz = ImMin(style.GrabMinSize, slider_sz);
	else
		grab_sz = ImMin(ImMax(1.f * (slider_sz / (v_max - v_min + 1.f)), style.GrabMinSize), slider_sz);	//Integer sliders, if possible have the grab size represent 1 unit
	const float slider_usable_sz = slider_sz - grab_sz;
	const float slider_usable_posMin = (is_horizontal ? frame_bb.Min.x : frame_bb.Min.y) + grab_padding + grab_sz*0.5f;
	const float slider_usable_posMax = (is_horizontal ? frame_bb.Max.x : frame_bb.Max.y) - grab_padding - grab_sz*0.5f;

	//For logarithmic sliders that cross over sign boundary we want the exponential increase to be symmetric around 0.f
	float linear_zero_pos = 0.f;	//0.0->1.f
	if (v_min * v_max < 0.f)
	{
		//Different sign
		const float linear_dist_min_to_0 = powf(fabsf(0.f - v_min), 1.f / power);
		const float linear_dist_max_to_0 = powf(fabsf(v_max - 0.f), 1.f / power);
	...
	if (g.ActiveId == id)
	{
		if (g.IO.MouseDown[0])
		{
			...
			float normalized_pos = ImClamp((mouse_abs_pos - slider_usable_posMin) / slider_usable_sz, 0.f, 1.f);
			if (!is_horizontal)
				normalized_pos = 1.f - normalized_pos;
			...
			if (is_non_linear)
			{
				...
				if (normalized_pos < linear_zero_pos)
				{
					...
					float a = 1.f - (normalized_pos / linear_zero_pos);
					a = powf(a, power);
				...
				else
				{
					...
					float a;
					if (fabsf(linear_zero_pos - 1.f) > 1.e-6)
						a = (normalized_pos - linear_zero_pos) / (1.f - linear_zero_pos);
					else
						a = normalized_pos;
					a = powf(a, power);
					new_value = ImLerp(ImMax(v_min, 0.f), v_max, a);
				}
			}
			else
			{
			...
			//Round past decimal precision
			new_value = RoundScalar(new_value, decimalPrecision);
			if (*v != new_value)
			{
				*v = new_value;
				valueChanged = true;
	...
	if (is_non_linear)
	{
		...
		if (v_clamped < 0.f)
		{
			const float f = 1.f - (v_clamped - v_min) / (ImMin(0.f, v_max) - v_min);
			grab_t = (1.f - powf(f, 1.f / power)) * linear_zero_pos;
		}
		else
		{
			const float f = (v_clamped - ImMax(0.f, v_min)) / (v_max - ImMax(0.f, v_min));
			grab_t = linear_zero_pos + powf(f, 1.f / power) * (1.f - linear_zero_pos);
		}
	}
	else
	{
	...
	if (!is_horizontal)
		grab_t = 1.f - grab_t;
	const float grab_pos = ImLerp(slider_usable_posMin, slider_usable_posMax, grab_t);
	ImRect grab_bb;
	if (is_horizontal)
...
//  "Gold: %.f"   Gold: 1
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* displayFormat, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y) + style.FramePadding*2.f);
	...
	if (start_text_input || (g.ActiveId == id && g.ScalarAsInputTextId == id))
		return InputScalarAsWidgetReplacement(frame_bb, label, ImGuiDataType_Float, v, id, decimalPrecision);

	ItemSize(total_bb, style.FramePadding.y);

	//Actual slider behavior + render grab
	const bool valueChanged = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimalPrecision);

	//Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_bufEnd = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), displayFormat, *v);
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_bufEnd, NULL, ImGuiAlign_Center | ImGuiAlign_VCenter);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* displayFormat, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	//Actual slider behavior + render grab
	bool valueChanged = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimalPrecision, ImGuiSliderFlags_Vertical);

	//Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	//For the vertical slider we allow centered text to overlap the frame padding
	char value_buf[64];
	char* value_bufEnd = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), displayFormat, *v);
	RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, value_buf, value_bufEnd, NULL, ImGuiAlign_Center);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* displayFormat)
{
	if (!displayFormat)
		displayFormat = "%.f";
	float v_f = (float)*v;
	bool valueChanged = ImGui::SliderFloat(label, &v_f, (float)v_min, (float)v_max, displayFormat, 1.f);
	*v = (int)v_f;
	return valueChanged;
}

bool ImGui::VSliderInt(const char* label, const ImVec2& size, int* v, int v_min, int v_max, const char* displayFormat)
{
	if (!displayFormat)
		displayFormat = "%.f";
	float v_f = (float)*v;
	bool valueChanged = ImGui::VSliderFloat(label, size, &v_f, (float)v_min, (float)v_max, displayFormat, 1.f);
	*v = (int)v_f;
	return valueChanged;
}

//Add multiple sliders on 1 line for compact edition of multiple components
bool ImGui::SliderFloatN(const char* label, float* v, int components, float v_min, float v_max, const char* displayFormat, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		ImGui::PushID(i);
		valueChanged |= ImGui::SliderFloat("##v", &v[i], v_min, v_max, displayFormat, power);
...
bool ImGui::SliderFloat2(const char* label, float v[2], float v_min, float v_max, const char* displayFormat, float power)
{
	return SliderFloatN(label, v, 2, v_min, v_max, displayFormat, power);
}

bool ImGui::SliderFloat3(const char* label, float v[3], float v_min, float v_max, const char* displayFormat, float power)
{
	return SliderFloatN(label, v, 3, v_min, v_max, displayFormat, power);
}

bool ImGui::SliderFloat4(const char* label, float v[4], float v_min, float v_max, const char* displayFormat, float power)
{
	return SliderFloatN(label, v, 4, v_min, v_max, displayFormat, power);
}

bool ImGui::SliderIntN(const char* label, int* v, int components, int v_min, int v_max, const char* displayFormat)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		ImGui::PushID(i);
		valueChanged |= ImGui::SliderInt("##v", &v[i], v_min, v_max, displayFormat);
		ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
		ImGui::PopID();
		ImGui::PopItemWidth();
	}
...
bool ImGui::SliderInt2(const char* label, int v[2], int v_min, int v_max, const char* displayFormat)
{
	return SliderIntN(label, v, 2, v_min, v_max, displayFormat);
}

bool ImGui::SliderInt3(const char* label, int v[3], int v_min, int v_max, const char* displayFormat)
{
	return SliderIntN(label, v, 3, v_min, v_max, displayFormat);
}

bool ImGui::SliderInt4(const char* label, int v[4], int v_min, int v_max, const char* displayFormat)
{
	return SliderIntN(label, v, 4, v_min, v_max, displayFormat);
}

bool ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimalPrecision, float power)
{
	ImGuiState& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	if (g.ActiveId == id)
	{
		if (g.IO.MouseDown[0])
		{
			...
			{
				float speed = v_speed;
				if (speed == 0.f && (v_max - v_min) != 0.f && (v_max - v_min) < FLT_MAX)
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* displayFormat, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool valueChanged = DragBehavior(frame_bb, id, v, v_speed, v_min, v_max, decimalPrecision, power);
...
bool ImGui::DragFloatN(const char* label, float* v, int components, float v_speed, float v_min, float v_max, const char* displayFormat, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		ImGui::PushID(i);
		valueChanged |= ImGui::DragFloat("##v", &v[i], v_speed, v_min, v_max, displayFormat, power);
		ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
		ImGui::PopID();
		ImGui::PopItemWidth();
	}
...
bool ImGui::DragFloat2(const char* label, float v[2], float v_speed, float v_min, float v_max, const char* displayFormat, float power)
{
	return DragFloatN(label, v, 2, v_speed, v_min, v_max, displayFormat, power);
}

bool ImGui::DragFloat3(const char* label, float v[3], float v_speed, float v_min, float v_max, const char* displayFormat, float power)
{
	return DragFloatN(label, v, 3, v_speed, v_min, v_max, displayFormat, power);
}

bool ImGui::DragFloat4(const char* label, float v[4], float v_speed, float v_min, float v_max, const char* displayFormat, float power)
{
	return DragFloatN(label, v, 4, v_speed, v_min, v_max, displayFormat, power);
}

bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* displayFormat, const char* displayFormat_max, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	bool valueChanged = ImGui::DragFloat("##min", v_current_min, v_speed, (v_min >= v_max) ? -FLT_MAX : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), displayFormat, power);
	ImGui::PopItemWidth();
	ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
	valueChanged |= ImGui::DragFloat("##max", v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? FLT_MAX : v_max, 
...
bool ImGui::DragInt(const char* label, int* v, float v_speed, int v_min, int v_max, const char* displayFormat)
{
	if (!displayFormat)
		displayFormat = "%.f";
	float v_f = (float)*v;
	bool valueChanged = ImGui::DragFloat(label, &v_f, v_speed, (float)v_min, (float)v_max, displayFormat);
	*v = (int)v_f;
	return valueChanged;
}

bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed, int v_min, int v_max, const char* displayFormat)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		ImGui::PushID(i);
		valueChanged |= ImGui::DragInt("##v", &v[i], v_speed, v_min, v_max, displayFormat);
		ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
		ImGui::PopID();
		ImGui::PopItemWidth();
	}
...
bool ImGui::DragInt2(const char* label, int v[2], float v_speed, int v_min, int v_max, const char* displayFormat)
{
	return DragIntN(label, v, 2, v_speed, v_min, v_max, displayFormat);
}

bool ImGui::DragInt3(const char* label, int v[3], float v_speed, int v_min, int v_max, const char* displayFormat)
{
	return DragIntN(label, v, 3, v_speed, v_min, v_max, displayFormat);
}

bool ImGui::DragInt4(const char* label, int v[4], float v_speed, int v_min, int v_max, const char* displayFormat)
{
	return DragIntN(label, v, 4, v_speed, v_min, v_max, displayFormat);
}

bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* displayFormat, const char* displayFormat_max)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	bool valueChanged = ImGui::DragInt("##min", v_current_min, v_speed, (v_min >= v_max) ? IM_INT_MIN : v_min, (v_min >= v_max) ? *v_current_max : ImMin(v_max, *v_current_max), displayFormat);
	ImGui::PopItemWidth();
	ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
	valueChanged |= ImGui::DragInt("##max", v_current_max, v_speed, (v_min >= v_max) ? *v_current_min : ImMax(v_min, *v_current_min), (v_min >= v_max) ? IM_INT_MAX : v_max, displayFormat_max ? displayFormat_max : displayFormat);
	ImGui::PopItemWidth();
	ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* textBegin, const ImWchar* textEnd, const ImWchar** remaining, ImVec2* outOffset, bool stopOnNewLine)
{
	...
	if (outOffset)
		*outOffset = ImVec2(line_width, textSize.y + line_height);	//offset allow for the possibility of sitting after a trailing \n
...
bool ImGui::InputTextEx(const char* label, char* buf, int bufSize, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* userData)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), is_multiline ? ImGui::GetTextLineHeight() * 8.f : label_size.y);	//Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size + style.FramePadding*2.f);
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.f ? (style.ItemInnerSpacing.x + label_size.x) : 0.f, 0.f));
	...
	//Render
	const ImVec4 clipRect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + size.x + style.FramePadding.x*2.f, frame_bb.Min.y + size.y + style.FramePadding.y*2.f);
	...
	if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY")))
	{
		...
		if (cursor_is_visible)
			draw_window->DrawList->AddLine(cursor_screen_pos + ImVec2(0.f, -g.FontSize + 0.5f), cursor_screen_pos + ImVec2(0.f, -1.5f), window->Color(ImGuiCol_Text));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int itemsCount, int height_in_items)
{
	...
	if (IsPopupOpen(id))
	{
		...
		float popup_height = (label_size.y + style.ItemSpacing.y) * ImMin(itemsCount, height_in_items) + (style.FramePadding.y * 3);
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
	...
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth() + style.FramePadding.x * 2.f, ImGui::GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	...
	float w = window->MenuColumns.DeclColumns(label_size.x, shortcut_size.x, (float)(int)(g.FontSize * 1.2f));	//Feedback for next frame
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	else
	{
		...
		float w = window->MenuColumns.DeclColumns(label_size.x, 0.f, (float)(int)(g.FontSize * 1.2f));	//Feedback to next frame
		float extra_w = ImMax(0.f, ImGui::GetContentRegionAvail().x - w);
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha)
{
	...
	case ImGuiColorEditMode_HSV:
	{
		...
		const float w_item_one = ImMax(1.f, (float)(int)((w_items_all - (style.FramePadding.x*2.f + style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
		const float w_item_last = ImMax(1.f, (float)(int)(w_items_all - (w_item_one + style.FramePadding.x*2.f + style.ItemInnerSpacing.x) * (components - 1)));
